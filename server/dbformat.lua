db = {
    mods = fw.qb and "mods" or "stored",
    owner = fw.qb and "citizenid" or "owner",
    state = fw.qb and "state" or "stored",
    players = fw.qb and "players" or "users",
    identifier = fw.qb and "citizenid" or "identifier",
    player_vehicles = fw.qb and "player_vehicles" or "owned_vehicles",
    
    player_name = {
        first = fw.qb and "JSON_VALUE(p.charinfo, '$.firstname')" or "p.firstname",
        last = fw.qb and "JSON_VALUE(p.charinfo, '$.lastname')" or "p.lastname"
    },

    select = {
        vehlist = fw.qb and [[
            pv.vehicle, pv.mods, pv.state, pv.depotprice, pv.plate, pv.fakeplate, pv.deformation
        ]] or [[
            pv.vehicle, pv.plate, pv.stored, pv.deformation
        ]],
    },

    filter = {
        plate = fw.qb and "plate = ? OR fakeplate = ?" or "plate = ?",
    },
